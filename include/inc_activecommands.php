<?php
	...
	if($_GET['t'] && $_GET['a'] && $USERINFO['admin'])
	{
		...
		if($ACTION == 'r')
		{
			mysqli_query($conn, 'DELETE FROM tasks_done WHERE taskid = \'' . mysql_real_escape_string($TASKID) . '\';');
		...
		{
			mysqli_query($conn, 'DELETE FROM tasks WHERE id = \'' . mysql_real_escape_string($TASKID) . '\';');
			mysqli_query($conn, 'DELETE FROM tasks_done WHERE taskid = \'' . mysql_real_escape_string($TASKID) . '\';');
	...
	if($_GET['all'] && $USERINFO['admin'])
	{
		if($_GET['all'] == 'c')
		{
			mysqli_query($conn, 'DELETE FROM tasks_done WHERE 1;');
			mysqli_query($conn, 'DELETE FROM tasks WHERE 1;');
	...
	$result = mysqli_query($conn, 'SELECT id, username, task, parameter, `limit` FROM tasks WHERE 1;');
	while($row = mysqli_fetch_array($result))
	{
		if(htmlentities($row['username']) == $USERINFO['username'] || $USERINFO['admin'])
		{
			$count = mysqli_num_rows(mysqli_query($conn, 'SELECT id FROM tasks_done WHERE taskid = \'' . mysql_real_escape_string($row['id']) . '\';'));
