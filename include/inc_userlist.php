<?php
	...
	if($_GET['d'])
	{
		$result = mysqli_query($conn, 'DELETE FROM users WHERE id = \'' . mysql_real_escape_string($_GET['d']) . '\';');
	...
	if($_POST['user_username'] && $_POST['user_password'] && $_POST['user_admin'])
	{
		...
		$result = mysqli_query($conn, 'INSERT INTO users SET ' . 
							  'username=\'' . mysqli_real_escape_string($conn, trim($_POST['user_username'])) . '\', ' . 
							  'password=\'' . $hashedpass . '\', ' . 
							  'lastip=\'N/A\', ' .
	...
	$result = mysqli_query($conn, 'SELECT * FROM users WHERE 1;');
