<?php
	...
	if($_POST['misc_task'])
	{
		...
		if($SUBMIT_TASK)
		{
			...
			{
				mysqli_query($conn, 'INSERT INTO tasks SET ' . 
	...
	$result = mysqli_query($conn, 'SELECT country FROM botlist WHERE 1 GROUP BY country;');
	while($row = mysqli_fetch_array($result)) $PAGE_INCLUDE_FILTER_COUNTRY .= '								<option value="' . $row['country'] . '">' . $row['country'] . '</option>' . "\n";
	
	$result = mysqli_query($conn, 'SELECT os FROM botlist WHERE 1 GROUP BY os;');
	while($row = mysqli_fetch_array($result)) $PAGE_INCLUDE_FILTER_OS .= '								<option value="' . htmlentities($row['os']) . '">' . htmlentities($row['os']) . '</option>' . "\n";
	
	$result = mysqli_query($conn, 'SELECT version FROM botlist WHERE 1 GROUP BY version;');
	while($row = mysqli_fetch_array($result)) $PAGE_INCLUDE_FILTER_BOTVERSION .= '								<option value="' . htmlentities($row['version']) . '">' . htmlentities($row['version']) . '</option>' . "\n";
	
	$result = mysqli_query($conn, 'SELECT net FROM botlist WHERE 1 GROUP BY net;');
	while($row = mysqli_fetch_array($result)) $PAGE_INCLUDE_FILTER_NET .= '								<option value="' . htmlentities($row['net']) . '">' . htmlentities($row['net']) . '</option>' . "\n";
?>