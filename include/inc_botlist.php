<?php
	...
	if($_GET['b'])
	{
		$result = mysqli_query($conn, 'SELECT * FROM botlist WHERE botid = \'' . mysqli_real_escape_string($conn, trim($_GET['b'])) . '\' LIMIT 1;');
	...
	$results = array();	
	$results[] = mysqli_query($conn, 'SELECT botid, country, country_code, ip, os, cpu, type, cores, net, admin, ram, version, lastseen FROM botlist WHERE lastseen >= \'' . mysqli_real_escape_string($TIME - ONLINE - 30) . '\' ORDER BY lastseen DESC;') or die(mysqli_error());
	$results[] = mysqli_query($conn, 'SELECT botid, country, country_code, ip, os, cpu, type, cores, net, admin, ram, version, lastseen FROM botlist WHERE lastseen < \'' . mysqli_real_escape_string($TIME - ONLINE - 30) . '\' AND lastseen >= \'' . mysqli_real_escape_string($TIME - DEAD - 30) . '\' ORDER BY lastseen DESC;') or die(mysqli_error());
	$results[] = mysqli_query($conn, 'SELECT botid, country, country_code, ip, os, cpu, type, cores, net, admin, ram, version, lastseen FROM botlist WHERE lastseen < \'' . mysqli_real_escape_string($TIME - DEAD - 30) . '\' ORDER BY lastseen DESC;') or die(mysqli_error());
